...
function GalaxyMap({ lessons, progress }) {
  ...
  const [tipIdx,  setTipIdx]  = useState(null); // index of tapped planet
  ...
  return (
    <div style={{marginBottom:16,background:C.card,borderRadius:16,overflow:"hidden",border:`1px solid ${C.purple}33`}}>
      <div onClick={()=>{setOpen(o=>!o);setTipIdx(null);}} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 14px",cursor:"pointer"}}>
              ...
              return (
                <React.Fragment key={l.id}>
                  <div style={{display:"flex",flexDirection:"column",alignItems:"center",position:"relative"}}>
                    <div onClick={()=>setTipIdx(isTip?null:i)} style={{
                      width:complete?52:started?46:38,height:complete?52:started?46:38,
                      borderRadius:"50%",cursor:"pointer",
                      background:complete?`radial-gradient(circle at 35% 35%,${col},${col}88)`:started?`radial-gradient(circle at 35% 35%,${col}88,${col}44)`:`radial-gradient(circle at 35% 35%,${C.dim}44,${C.dim}22)`,
                      boxShadow:complete?`0 0 16px ${col}88`:started?`0 0 8px ${col}44`:"none",
                      border:`2px solid ${complete?col:started?col+"88":C.dim+"33"}`,
                      display:"flex",alignItems:"center",justifyContent:"center",fontSize:complete?18:started?14:12,
                      transition:"all 0.2s",position:"relative",
                    }}>
                      {complete?"⭐":started?l.emoji||"📚":"🔒"}
                      {started&&!complete&&<div style={{position:"absolute",bottom:-2,right:-2,background:C.yellow,borderRadius:"50%",width:14,height:14,display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,color:"#000",fontWeight:800}}>{pct}%</div>}
                    </div>
                    <div style={{fontSize:9,color:complete?col:started?col:C.dim,marginTop:4,fontFamily:"'Orbitron',sans-serif"}}>L{i+1}</div>
                    {isTip&&(
                      <div style={{position:"absolute",top:62,left:"50%",transform:"translateX(-50%)",background:C.card2,border:`1px solid ${col}44`,borderRadius:12,padding:"8px 10px",zIndex:10,minWidth:130,boxShadow:"0 4px 20px rgba(0,0,0,0.5)"}}>
                        <div style={{fontSize:11,fontWeight:800,color:"white",marginBottom:3}}>{l.emoji} {l.title}</div>
                        ...
                        <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:2}}>
                          {Array.from({length:20},(_,si)=>{
                            ...
                            return <div key={si} style={{height:8,borderRadius:2,background:d?C.green:`${C.dim}33`}}/>;
                          })}
                        </div>
                      </div>
                    )}
                  ...
                  {i<lessons.length-1&&(
                    <div style={{display:"flex",alignItems:"center",marginTop:-8}}>
                      {[0,1,2].map(d=><div key={d} style={{width:8,height:3,borderRadius:2,background:done>0?`${col}66`:C.dim+"22",margin:"0 1px"}}/>)}
                    </div>
                  )}
                </React.Fragment>
              );
