...
function getDailyStreak(childId) { return parseInt(localStorage.getItem(`bz_dstreak_${childId}`)||"0"); }
...
function getBazaarOutfit(childId) {
...
function setBazaarOutfit(childId, outfit) { localStorage.setItem(`bz_outfit_${childId}`, JSON.stringify(outfit)); }
function getBazaarPurchased(childId) {
  try { return JSON.parse(localStorage.getItem(`bz_purchased_${childId}`)||"[]"); }
  catch { return []; }
}
function addBazaarPurchase(childId, itemId) {
...
function isItemOwned(childId, itemId) { return itemId===undefined||BAZAAR_AVATAR_ITEMS.hat.find(i=>i.id===itemId)?.price===0 || getBazaarPurchased(childId).includes(itemId); }

// ─── PHASE 4: Achievements helpers ───────────────────────────────────
function getBazaarStats(childId) {
...
function updateBazaarStats(childId, delta) {
...
function generateChallengeId() { return Math.random().toString(36).slice(2,10).toUpperCase(); }
function saveChallengeResult(challengeId, score, total, childName) {
...
function getSpeedRoundQuestions(market) {
  return Array.from({length:12},(_,i)=>{
    ...
    return {scenario:`${item.emoji} ${qty} × ${item.name}`,question:`= ₹?`,hint:`${qty}×₹${item.price}`,options:[String(ans-10),String(ans),String(ans+10),String(ans+20)].sort(()=>Math.random()-0.5),correct_answer:String(ans),customer_name:"",customer_emoji:BAZAAR_CUSTOMER_EMOJIS[i%BAZAAR_CUSTOMER_EMOJIS.length]};
  });
}
    ...
    <div style={{display:"flex",gap:5,alignItems:"center",justifyContent:"center",padding:"5px 0"}}>
      {Array.from({length:Math.min(total,8)}).map((_,i)=>(
        <div key={i} style={{position:"relative"}}>
          <div style={{fontSize:20,filter:i<current?"grayscale(1) opacity(0.3)":i===current?"none":"opacity(0.65)",transform:i===current?"scale(1.22)":"scale(1)",transition:"all 0.3s"}}>{customerEmojis[i%customerEmojis.length]}</div>
...
function BazaarAvatar({ outfit, size=80 }) {
  ...
  return (
    <div style={{position:"relative",width:size,height:size,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
      ...
      {/* Main body */}
      <div style={{width:size*0.72,height:size*0.72,borderRadius:size*0.2,background:`linear-gradient(135deg,${C.purple}33,${C.cyan}22)`,border:`2.5px solid ${C.purple}55`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:size*0.42}}>🧑‍🛒</div>
...
// ═══════════════════════════════════════════════════════════════════════
//  PHASE 3: Character Customisation screen
// ═══════════════════════════════════════════════════════════════════════
function BazaarCustomise({ child, onClose }) {
  ...
  const [bought,  setBought]  = useState(null); // flash message
        ...
        <div style={{display:"flex",justifyContent:"center",marginBottom:16}}>
          <div style={{background:`linear-gradient(135deg,${C.purple}22,${C.cyan}14)`,border:`2px solid ${C.purple}44`,borderRadius:24,padding:"20px 30px",display:"flex",flexDirection:"column",alignItems:"center",gap:8}}>
            ...
            <div style={{fontSize:12,color:C.dim,textAlign:"center"}}>
              {[outfit.hat,outfit.outfit,outfit.accessory,outfit.shop_sign].map(id=>{
                const item = Object.values(BAZAAR_AVATAR_ITEMS).flat().find(i=>i.id===id);
                return item?.emoji||"";
              }).join(" ")}
            </div>
          </div>
          ...
          {currentItems.map((item,i)=>{
            ...
            return (
              <button key={item.id} onClick={()=>handleBuy(item)}
                ...
                {item.price===0
                  ? <div style={{fontSize:9,color:C.green,fontWeight:700}}>FREE</div>
                  : owned
                    ? <div style={{fontSize:9,color:C.green,fontWeight:700}}>✅ Owned</div>
...
// ═══════════════════════════════════════════════════════════════════════
//  PHASE 4: Achievements / Badges Wall
// ═══════════════════════════════════════════════════════════════════════
          ...
          {BAZAAR_ACHIEVEMENTS.map((a,i)=>{
            ...
            return (
              <div key={a.id} style={{background:isEarned?`${a.color}16`:isDark()?"rgba(255,255,255,0.04)":"rgba(0,0,0,0.04)",border:`1.5px solid ${isEarned?a.color+"55":a.color+"18"}`,borderRadius:16,padding:"12px 8px",textAlign:"center",opacity:isEarned?1:0.5,transition:"all 0.2s",animation:isEarned?`popIn 0.4s ease ${(i%6)*0.05}s both`:"none"}}>
                ...
                {isEarned
                  ? <div style={{fontSize:8,color:C.dim,marginTop:3,lineHeight:1.4}}>{a.desc}</div>
...
function ChallengeMode({ market, child, onBack, onDone }) {
  const [phase,     setPhase]     = useState("intro");   // intro | playing | share
  ...
  function handleAnswer(opt) {
    ...
    if(correct){SFX.correct();setScore(s=>s+1);setFeedback("correct");setTimeout(()=>{if(qIdx+1>=questions.length){saveChallengeResult(challengeId,score+1,questions.length,child.name);const newStats=updateBazaarStats(child.id,{challengesSent:1});setPhase("share");}else setQIdx(i=>i+1);},1200);}
      ...
      <div style={{position:"relative",zIndex:2,width:"100%",maxWidth:380}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          ...
          <div style={{background:C.card,border:`1.5px solid ${C.cyan}33`,borderRadius:16,padding:"11px 14px",display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{flex:1}}>
              <div style={{fontSize:10,color:C.dim}}>Challenge Code</div>
              <div style={{fontSize:16,fontWeight:900,color:C.cyan,fontFamily:"'Orbitron',sans-serif"}}>{challengeId}</div>
            </div>
    ...
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Baloo 2','Nunito',sans-serif",paddingBottom:28,position:"relative",overflow:"hidden"}}>
      ...
      <div style={{background:`linear-gradient(135deg,${C.cyan}22,${C.purple}0a)`,borderBottom:`2px solid ${C.cyan}44`,padding:"13px 15px"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",marginTop:3,fontSize:10,color:C.dim}}>
          <span>Q {qIdx+1}/{total}</span><span>Challenge ID: <b style={{color:C.cyan}}>{challengeId}</b></span>
        </div>
      </div>
...
// ═══════════════════════════════════════════════════════════════════════
//  PHASE 5: New Achievement Toast (shows after a session)
// ═══════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════
//  BazaarHub — master hub with all Phase 3-5 entry points
// ═══════════════════════════════════════════════════════════════════════
function BazaarHub({ child, onBack, onMarket, onDaily, onSpeed, onChallenge }) {
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Baloo 2','Nunito',sans-serif",paddingBottom:50,position:"relative",overflowX:"hidden"}}>
      ...
      {/* ── Header ── */}
      <div style={{background:"linear-gradient(135deg,#f9731622,#ec489914)",borderBottom:"2px solid #f9731633",padding:"15px 15px 12px",position:"relative",zIndex:2}}>
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <button onClick={()=>setShowCustomise(true)} style={{background:"none",border:"none",cursor:"pointer",padding:0}}>
              <BazaarAvatar outfit={outfit} size={44}/>
            </button>
            <button onClick={()=>setShowLeague(true)} style={{background:"#fbbf2418",border:"1.5px solid #fbbf2444",borderRadius:12,padding:"5px 9px",cursor:"pointer",textAlign:"center"}}>
              <div style={{fontSize:13,fontWeight:900,color:"#fbbf24"}}>🪙{totalCoins}</div>
              <div style={{fontSize:7,color:C.dim}}>COINS</div>
            </button>
          </div>
        </div>
        ...
        {/* Festival banner */}
        {festival&&(
          <div style={{background:`linear-gradient(135deg,${festival.color}28,${festival.color}10)`,border:`2px solid ${festival.color}66`,borderRadius:14,padding:"8px 12px",marginBottom:8,display:"flex",alignItems:"center",gap:10,animation:"heartbeat 2s ease-in-out infinite"}}>
            <span style={{fontSize:24}}>{festival.emoji}</span>
            <div style={{flex:1}}><div style={{fontSize:12,fontWeight:900,color:festival.color}}>{festival.name}</div><div style={{fontSize:10,color:C.dim}}>{festival.desc}</div></div>
            <div style={{background:festival.color,borderRadius:9,padding:"2px 8px",fontSize:11,fontWeight:900,color:"white"}}>{festival.bonus}× 🪙</div>
          </div>
        )}
        ...
        <div style={{background:isDark()?"rgba(255,255,255,0.05)":"rgba(0,0,0,0.04)",borderRadius:13,padding:"9px 13px",display:"flex",gap:10,alignItems:"center"}}>
          <div style={{fontSize:34,animation:"floatUp 2.5s ease-in-out infinite",flexShrink:0}}>🧑‍🛒</div>
          <div style={{flex:1}}>
      ...
      <div style={{padding:"12px 14px",position:"relative",zIndex:2}}>
        ...
        {/* ── Quick actions row: Passport · League · Badges · Customise ── */}
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginBottom:13}}>
          {[
            ...
            {icon:"🏆",label:"League",    action:()=>setShowLeague(true),    color:C.yellow, sub:<LeagueBadge coins={weeklyCoins}/>},
            {icon:"🏅",label:"Badges",    action:()=>setShowBadges(true),    color:"#f97316", sub:`${earned.length}/${BAZAAR_ACHIEVEMENTS.length}`},
            {icon:"🎨",label:"Customise", action:()=>setShowCustomise(true), color:C.purple,  sub:"My Trader"},
          ].map((btn,i)=>(
            <button key={i} onClick={btn.action}
              style={{background:`${btn.color}14`,border:`1.5px solid ${btn.color}44`,borderRadius:16,padding:"10px 6px",cursor:"pointer",textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
              <div style={{fontSize:22}}>{btn.icon}</div>
              <div style={{fontSize:10,fontWeight:900,color:textColor()}}>{btn.label}</div>
              <div style={{fontSize:9,color:btn.color}}>{btn.sub}</div>
            </button>
          ))}
        </div>
        ...
        <div style={{marginBottom:11}}>
          ...
          <button onClick={()=>!dailyDone&&onDaily(todayChallenge)} disabled={dailyDone}
            style={{width:"100%",background:dailyDone?`${C.dim}0a`:`linear-gradient(135deg,${todayChallenge.color}26,${todayChallenge.color}0c)`,border:`2.5px solid ${dailyDone?C.dim+"22":todayChallenge.color+"66"}`,borderRadius:20,padding:"13px 16px",cursor:dailyDone?"not-allowed":"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:12,position:"relative",overflow:"hidden"}}>
            {!dailyDone&&<div style={{position:"absolute",top:7,right:11,background:todayChallenge.color,borderRadius:9,padding:"2px 8px",fontSize:9,fontWeight:900,color:"white",animation:"heartbeat 1.8s ease-in-out infinite"}}>NEW</div>}
            {dailyDone &&<div style={{position:"absolute",top:7,right:11,background:"#22c55e22",border:"1px solid #22c55e44",borderRadius:9,padding:"2px 8px",fontSize:9,fontWeight:900,color:"#22c55e"}}>✅ DONE</div>}
            <div style={{fontSize:40,animation:dailyDone?"none":"floatUp 2s ease-in-out infinite",filter:dailyDone?"grayscale(1) opacity(0.5)":"none"}}>{todayChallenge.emoji}</div>
            <div style={{flex:1}}>
              ...
              {!dailyDone&&<div style={{display:"flex",gap:5,marginTop:5}}><span style={{background:"#fbbf2420",border:"1px solid #fbbf2440",borderRadius:8,padding:"2px 8px",fontSize:10,fontWeight:900,color:"#fbbf24"}}>+{todayChallenge.bonus}× coins</span><span style={{background:"#22c55e20",border:"1px solid #22c55e40",borderRadius:8,padding:"2px 8px",fontSize:9,fontWeight:700,color:"#22c55e"}}>🏅 Badge</span></div>}
              {dailyDone&&<div style={{fontSize:10,color:C.dim,marginTop:4}}>Come back tomorrow for a fresh challenge!</div>}
            </div>
          </button>
        </div>

        {/* ── Speed Round + Challenge a Friend row ── */}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
          <button onClick={()=>onSpeed(BAZAAR_MARKETS.find(m=>!m.isPaid))}
            style={{background:"linear-gradient(135deg,#ef444418,#f9731610)",border:"2px solid #ef444444",borderRadius:18,padding:"12px 12px",cursor:"pointer",textAlign:"left"}}>
            <div style={{width:38,height:38,borderRadius:11,background:"linear-gradient(135deg,#ef4444,#f97316)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,marginBottom:6}}>⚡</div>
            <div style={{fontSize:13,fontWeight:900,color:textColor()}}>Speed Round</div>
            <div style={{fontSize:10,color:C.dim,marginTop:2}}>60 secs · fast fire!</div>
          </button>
          <button onClick={()=>onChallenge(BAZAAR_MARKETS.find(m=>!m.isPaid))}
            style={{background:`linear-gradient(135deg,${C.cyan}18,${C.purple}08)`,border:`2px solid ${C.cyan}44`,borderRadius:18,padding:"12px 12px",cursor:"pointer",textAlign:"left"}}>
            <div style={{width:38,height:38,borderRadius:11,background:`linear-gradient(135deg,${C.cyan},${C.purple})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,marginBottom:6}}>📤</div>
            <div style={{fontSize:13,fontWeight:900,color:textColor()}}>Challenge!</div>
            <div style={{fontSize:10,color:C.dim,marginTop:2}}>Beat a friend!</div>
          </button>
        </div>
          ...
          {BAZAAR_MARKETS.map((m,i)=>{
            ...
            return (
              <button key={m.id} onClick={()=>!m.isPaid&&onMarket(m)}
                ...
                {!m.isPaid&&(
                  <><div style={{display:"flex",alignItems:"center",gap:5}}><span style={{fontSize:12}}>{m.keeperEmoji}</span><span style={{fontSize:10,color:m.color,fontWeight:700}}>{m.keeper}</span></div>
    ...
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Baloo 2','Nunito',sans-serif",paddingBottom:40,overflowX:"hidden"}}>
      <div style={{background:`linear-gradient(135deg,${market.color}22,${market.color}0a)`,borderBottom:`2px solid ${market.color}44`,padding:"15px 15px 12px"}}>
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}>
          ...
          <div style={{flex:1}}><div style={{fontSize:9,color:market.color,fontWeight:900,letterSpacing:2,fontFamily:"'Orbitron',sans-serif"}}>{isDaily?"🌟 DAILY CHALLENGE":"BAZAAR MATHS"}</div><div style={{fontSize:19,fontWeight:900,color:textColor()}}>{market.name}</div></div>
...
function BazaarGame({ market, role, child, onBack, onDone, isDaily, dailyChallenge }) {
  ...
  useEffect(()=>{ fetchBazaarQuestions(market.id,role,isDaily).then(data=>{ setQuestions(data||getBazaarFallback(market,role)); setLoading(false); }); },[market.id,role]);
  useEffect(()=>{ if(questions.length>0&&questions[qIdx]) setShuffled([...(questions[qIdx].options||[])].sort(()=>Math.random()-0.5)); setFeedback(null); },[qIdx,questions]);
